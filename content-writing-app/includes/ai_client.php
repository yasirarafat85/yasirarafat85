...
function ai_chat(int $userId, string $system, string $userPrompt, ?string $modelOverride = null, float $temperature = 0.7, int $maxTokens = 2000): array
{
    $s = get_ai_settings($userId);
    if ($s['api_key'] === '') {
    ...
    $fn = $map[$provider];
    return $fn($s['api_key'], $model, $system, $userPrompt, $temperature, $maxTokens);
}
